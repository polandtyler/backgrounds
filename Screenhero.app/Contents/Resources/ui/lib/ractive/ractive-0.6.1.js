		...
		Decorator.prototype = {
			...
			teardown: function( updating ) {
				...
				if (this.ready)
					...
					console.log('not tearing down because not ready: '+this.element.node.attributes['data-title']);
	...
	var virtualdom_items_Element$render = function( namespaces, isArray, warn, create, createElement, defineProperty, noop, runloop, getInnerContext, renderImage, Transition ) {
		...
		__export = function Element$render() {
			...
			if ( this.decorator && this.decorator.fn ) {
				// console.log('scheduling decorator init: ' + this.decorator.element.attributes['data-title'].value);
				runloop.scheduleTask( function() {
					if (!this$0.decorator.tornapart)
						return this$0.decorator.init();
					else
						console.log('tooltip already torn apart, dont leak');
				}, true );
			}
